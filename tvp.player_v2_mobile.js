;(function (global) {
	 ...
	 * @type String
	 */
	tvp.lastModify = "2015-02-05 19:48:01";
	...
	typeof window.DEBUG == "undefined" && (window.DEBUG = 1);
	...
	tvp.log = function(msg) {
		if (window.DEBUG && document.getElementById('tvp_debug_console') != null) {
	...
	tvp.debug = function(msg) {
		if (!window.DEBUG && tvp.log.isDebug === -1) {
			tvp.log.isDebug = tvp.$.getUrlParam("debug") == "true" ? 1 : 0;
		}
		if (window.DEBUG || !! tvp.log.isDebug) {
			tvp.log(msg);
		}
	}
	/**
	...
	(function(tvp, $) {
		...
		tvp.app = {
			...
			bindTryOpenAppBanner: function(banner) {
				...
				// ios safari下，触碰到按钮就会触发touchend
				// fix this bug: 换成click
				clickEvent = $.os.ios ? 'click' : clickEvent;
				$btn.bind(clickEvent, function(e) {
					if(!banner.noTryOpen){
			 ...
			 * isNeedTime, 非必须
			 */
			getQQBrowserSignKey: function(vid, isNeedTime){
				var self = tvp.app,
					pageType = self.pageType,
					defer = new $.Deferred(),
					loadJSApi = this.loadMqqAPI();
				// 不是QQ浏览器，退出
				if(pageType!=3){
					defer.resolve(0);
					return;
				}
				loadJSApi.done(function(){
					if(!window.x5 || !tvp.$.os.ios){
						defer.resolve(0);
						return;
					}
					...
					var option = {vid: vid};
					if(isNeedTime){
						option.timestamp = parseInt(new Date().valueOf()/1000);
					}
					// alert(x5.ios.getBrowserSignature);
					// alert(JSON.stringify(option));
					x5.ios.getBrowserSignature(option, function(data){
						if(data && data.key && data.ver && data.platform){
							defer.resolve({
								key: data.key,
								ver: data.ver,
		...
		tvp.VideoInfo = function() {
			...
			this.getMP4Url = function(v) {
				...
				tvp.h5Helper.loadVideoUrlByVid({
					vid: vid,
					isPay: this.getPay(),
					eduext: privData.eduext ? privData.eduext:0,
					fmt: this.getFormat(),
	...
	(function(tvp, $) {
		...
		tvp.h5Helper = {
			...
			loadVideoUrlByVid_base : function(cfg, rs) {
				...
				// 腾讯课堂鉴权
				if (cfg.eduext) {
					getinfocgi = globalCfg.cgi.edugetvinfo;
				}
	        ...
	        _rnd : new Date().valueOf()
				...
				var pageType = tvp.app.pageType;
				var isIframe = window !== top;
				if(pageType===3 && !isIframe){
					// qq浏览器鉴权
					if(!cfg.qbAuthTimes){
						cfg.qbAuthTimes = 1;
					}
					tvp.app.getQQBrowserSignKey(s.vid, cfg.needTime).done(function(data){
						if(data){
							$.extend(baseParams, {
								platform: data.platform,
								cKey: data.key,
	...
	tvp.html5skin = {
		...
		defaultError: (function() {
			return [
				...
				// '<div class="tvp_live_download_app" id="$ERROR-TIPS-INNER$">',
				// '	<a class="tvp_download_app_inner" href="${url}">',
				// '		<i class="tpv_icon_download"></i>',
				// '		<span class="tvp_icon_text">$ERROR-MSG$ $ERROR-DETAIL$</span>',
				// '	</a>',
				// '</div>'
		...
		errorDownloader:(function() {
			return [
				...
				'<div class="tvp_live_download_app">',
				'	<a class="tvp_download_app_inner" href="${url}">',
				'		<i class="tpv_icon_download"></i>',
				'		<span class="tvp_icon_text">${errorMsg}</span>',
				'	</a>',
		...
		durationLimit_v2_case1: (function(){
			return [
				'<div class="tvp_layer_replay tvp_none" data-role="tvp-limit-replay">',
					'<div class="tvp_overlay_replay">',
						'<span class="tvp_button_replay"></span>',
						'<span class="tvp_text" data-role="tvp-limit-replay-text">${replayText}</span>',
					'</div>',
				'</div>',
				'<div class="tvp_overlay_play_try tvp_none" data-role="tvp-limit-play">',
					'<span class="tvp_button_play"></span>',
					'<span class="tvp_text" data-role="tvp-limit-play-text">${playText}</span>',
				'</div>',
				'<div class="tvp_app_banner tvp_none" data-role="tvp-limit-download">',
					'<a data-role="tvp-limit-download-btn" href="javascript:;" class="tvp_app_btn" data-status="down">',
						'<span class="tvp_progress">',
							'<span class="tvp_progress_current"></span>',
						'</span>',
						'<span data-role="tvp-limit-download-text" class="tvp_text" data-text="${download}"></span>',
					'</a>',
				'</div>',
				'<div class="tvp_app_badge" data-role="tvp-limit-timing">',
					'<a href="javascript:;" data-role="tvp-limit-timing-btn" class="tvp_app_btn tvp_none" data-status="down">',
						'<span class="tvp_progress"><span class="tvp_progress_current"></span></span>',
						'<span class="tvp_text" data-text="还剩" data-role="tvp-limit-timing-text"></span>',
					'</a>',
				'</div>'
			].join("");
		})(),
		durationLimit_v2_case2: (function(){
			return [
				'<div class="tvp_external_layer tvp_none" data-role="tvp-limit-main">',
					'<div class="tvp_external_inner">',
						'<div class="tvp_overlay_play_try" data-role="tvp-limit-play">',
							'<span class="tvp_button_play"></span>',
							'<span class="tvp_text" data-role="tvp-limit-play-text">${playText}</span>',
						'</div>',
						'<div class="tvp_layer_replay tvp_none" data-role="tvp-limit-replay">',
						...
						'</div>',
						'<div class="tvp_app_banner tvp_none" data-role="tvp-limit-download">',
							'<a href="javascript:;" class="tvp_app_btn" data-status="down" data-role="tvp-limit-download-btn">',
								'<span class="tvp_progress"><span class="tvp_progress_current"></span></span>',
								'<span data-role="tvp-limit-download-text" class="tvp_text" data-text="${download}"></span>',
							'</a>',
						'</div>',
				...
				'</div>',
				'<div class="tvp_app_badge" data-role="tvp-limit-timing">',
					'<a data-role="tvp-limit-timing-btn" href="javascript:;" class="tvp_app_btn tvp_none" data-status="down">',
						'<span class="tvp_text" data-text="还剩" data-role="tvp-limit-timing-text"></span>',
					'</a>',
				'</div>'
			].join("");
		...
		durationLimit_v2_case3: (function(){
			return [
				'<div class="tvp_app_badge" data-role="tvp-limit-timing">',
					'<a data-role="tvp-limit-timing-btn" href="javascript:;" class="tvp_app_btn tvp_none" data-status="down">',
						'<span class="tvp_progress"><span class="tvp_progress_current"></span></span>',
						'<span data-role="tvp-limit-timing-text" class="tvp_text" data-text="${timingText}"></span>',
					'</a>',
				'</div>'
			].join("");
		})(),
	...
	(function(tvp, $) {
		...
		$.extend(tvp.BaseHtml5.fn, {
			...
			createVideoHtml: function() {
				if(!$.browser.WeChat){
					this.config.isiPhoneShowPlaysinline = false;
				}
			...
			resize: function(width, height) {
				...
				if($box){
					...
					$box && $box.trigger && $box.trigger('tvp:resize');
				}
			},
			/**
			 * 显示播放器播放出错
	...
	//		cbTimer = setTimeout(cb, timeStep);
		...
		function Html5TinyPlayer(vWidth, vHeight) {
			...
			$.extend(this.h5EvtAdapter, {
				"onEnded": function() {
					...
					this.$video && this.$video.trigger && this.$video.trigger("tvp:player:allended"); //触发自定义的全部播放完毕事件
		...
		$.extend(Html5TinyPlayer.prototype, {
			...
			init_report_playtime: function(){
				if(this.has_report_playtime){
					return;
				}
				this.has_report_playtime = true;
	
				var time = 0,
					self = this,
					dataId = 'tvp_report_h5_playtime',
					data = $.getData(dataId),
					data_vid,
					data_time,
					is_pause = 1,
					timer,
					vid = this.curVideo.getVid();
				var is_ad = function(){
					if($.isFunction(self.isPlayingLoadingAd) &&
						self.isPlayingLoadingAd()){
						return true;
					}
					return false;
				}
				...
				var time_stop = function(){
					timer && clearInterval(timer);
					timer = null;
				};
				var report = function(vid, ptime){
					self.report_playtime({vid: vid, ptime: ptime});
					$.setData(dataId,0);
				}
				...
				if(data && data.indexOf('|')>-1){
					...
					data_vid = data[0];
					data_time = data[1];
					report(data_vid, data_time);
				}
			},
	
			report_playtime: function(opt){
				var params = {
					ptime: opt.ptime,
					url: opt.url || top.location.href,
					platform: tvp.common.getDeviceId(),
					vid: opt.vid
				};
				var g_btrace_BOSS = new Image();
				var url = "http://btrace.qq.com/kvcollect?BossId=2803&Pwd=524422958&_dc=" + Math.random();
				url += "&" + $.param(params);
				g_btrace_BOSS.src = url;
			}
		});
		...
		$.extend(Html5TinyPlayer.prototype, {
			...
			play: function(v, isAutoPlay, isUseHLS) {
				...
				function _play(isUseHls) {
					...
					t.$video && t.$video.trigger && t.$video.trigger("tvp:video:ajaxstart", v instanceof tvp.VideoInfo ? v.getVid() : v, isUseHls);
					...
					defer.done(function(videourl){
						videoUrl = videourl;
						t.$video && t.$video.trigger && t.$video.trigger("tvp:video:ajaxsuc", videourl);
						//显示限播提示
					...
					$.when(defer,loadingAdDefer).done(function(videourl) {
						...
						t.$video && t.$video.trigger && t.$video.trigger("tvp:video:src"); //触发自定义事件，video的src设置
						...
						t.$video && t.$video.trigger && t.$video.trigger("tvp:video:ajaxerror");
						t.$video && t.$video.trigger && t.$video.trigger("tvp:video:error", errcode, errcontent);
		...
		function Html5LiveTiny(vWidth, vHeight) {
			...
			$.extend(this.h5EvtAdapter, {
				...
				"onPlaying": function() {
					...
					if(!this.hasReportH5Live){
						this.hasReportH5Live = true;
						this.report_time.onplaying = new Date().valueOf();
						this.report_h5_live({
							firstload:(t.report_time.onplaying-t.report_time.onplay)/1000,
							playno: playno,
							blocktime: 0,
							block: 0,
							prd: 0
		...
		$.extend(Html5LiveTiny.fn, {
			...
			report_event: function(){
				...
				var timer = setInterval(function(){
					...
					c_time = v_time>c_time?v_time:c_time;
					// 60秒为一个周期做一次上报
					if(index === 60){
						waitime = waitime > 60 ? 60 : waitime;
						self.report_h5_live({
							prd: playtime,
							blocktime: waitime,
							block: waitimes,
			...
			// 直播质量上报
			report_h5_live: function( options ){
				options =  options || {};
				var getMp4Key = function() {
					var flag1 = location.host === 'view.inews.qq.com';
					if ($.os.iphone || $.os.ipod) {
						return flag1 ? "v3110" : "v3010";
					}
				...
				var video = this.getCurVideo(),
					userId = 'tvp_report_h5_live',
					prog = video.getChannelId(),
					guid = $.getData(userId),
					cdn = tvp.$.getUrlParam('cdn',self.$video.attr('src')),
					sdtfrom = getMp4Key(),
					platform = tvp.common.getDeviceId(),
	
					// 上报变动参数
					playno = options.playno || tvp.$.createGUID(),
					firstload = isUndefined(options.firstload),
					errcode = isUndefined(options.errcode),
					blocktime = isUndefined(options.blocktime),
					block = isUndefined(options.block),
					prd = isUndefined(options.prd);
				...
				var g_btrace_BOSS = new Image();
				var url = "http://btrace.qq.com/kvcollect?BossId=2727&Pwd=1424084450&_dc=" + Math.random();
				url += "&" + $.param(params);
				g_btrace_BOSS.src = url;
			}
		});
		...
		$.extend(LiveDownloader.prototype,{
			...
			_bindEvents:function(){
				...
				this.$btn.on(et,function(e){
					self._report(1);
					if(banner){
						banner.$btn && banner.$btn.trigger && banner.$btn.trigger(et);
					}						
				});
			},
	...
	(function(tvp, $) {
		...
		var eventname = $.os.hasTouch ? 'touchend' : 'click';
		var hideCls = 'tvp_none';
		var is_pad = $.os.ipad || $.os.tablet;
		...
		$.extend(DurationLimit.prototype, {
			build : function(t) {
				...
				if(enable){
					this.enable = true;
					this.timeRemainVal = data.preview;
					t.trigger && t.trigger('tvp.durationlimit.enable');
				}
				this.info = {};
				this.load_config();
			},
			...
			load_config: function(){
				var self = this;
				var limit_conf = 'http://v.qq.com/iframe/limit_conf.js';
				try{
					$.getScript(limit_conf, function() {
						self.build_conf();
						if(!self.limitCase){
							return;
						}
						// case3 如果有下载banner
						// if(self.limitCase===3 && self.hasAppBanner()){
						// 	return;
						// }
						// case3 灰度设置
						if(self.limitCase===3){
							var rnd = parseInt(Math.random()*100, 10)+1;
							if(self.op.hasOwnProperty('per')&& rnd > self.op.per){
								return;
							}
							if(self.hasAppBanner()){
								return;	
							}
						}
						self.init();
			...
			build_conf: function(){
				...
				// this.limitCase=3;
				// autopage配置
				var g_conf = window.g_limit_conf || {};
				// 用户参数配置
				var u_conf = this.t.configs || {};
				// 默认配置
				var b_conf = defaultConfig[ this.limitCase ];
				g_conf = g_conf[ this.limitCase ];
				this.op = {};
				$.extend(this.op, b_conf, g_conf, u_conf);
				this.op.appName = tvp.app.config.defaultName;
				this.op.name = defaultConfig.name;
				this.op.downloader = defaultConfig.downloader;
				this.op.range = defaultConfig.range;
			},
	
			init: function(){
				var self = this;
				var render = tvp.html5skin[defaultConfig.name + '_case'+this.limitCase];
				var renderData = this.op.texts;
				var data = this.t.curVideo.data;
				var limitTime = data.preview - 29;
				limitTime = limitTime>0 ? limitTime: 1;
				renderData.limitTime = Math.ceil(limitTime / 60);
				renderData.playText = $.formatTpl(renderData.playText, renderData);
				render = $.formatTpl(render, renderData);
				this.op.texts = renderData;
				this.$mod = this.t.$UILayer || this.t.$videomod;
				this.$mod.addClass('tvp_container');
				this.element = $(render).appendTo(this.$mod);
				this.setRole();
				this.initStyle();
				this._bindEvents();
				this.videoListener();
				if(this.limitCase===1 || this.limitCase===2){
					// 如果有暂停banner，不需要计算倒计时
					!self.hasPauseBanner() && setInterval(function(){
						if(self.info.has_click_download){
							return;
						}
						self.timeRemain();
					},500);
			...
			setRole: function(){
				var $null = tvp.$([]);
				this.$play = this.$mod.find('[data-role=tvp-limit-play]') || $null;
				this.$replay = this.$mod.find('[data-role=tvp-limit-replay]') || $null;
				this.$time = this.$mod.find('[data-role=tvp-limit-timing]') || $null;
				this.$time_btn = this.$mod.find('[data-role=tvp-limit-timing-btn]') || $null;
				this.$time_text = this.$mod.find('[data-role=tvp-limit-timing-text]') || $null;
				this.$download = this.$mod.find('[data-role=tvp-limit-download]') || $null;
				this.$download_btn = this.$mod.find('[data-role=tvp-limit-download-btn]') || $null;
				this.$download_text = this.$mod.find('[data-role=tvp-limit-download-text]') || $null;
				this.$case2_main = this.$mod.find('[data-role=tvp-limit-main]') || $null;
				this.$btns = $([this.$download_btn.get(0), this.$time_btn.get(0)]) || $null;
				this.$boxs = $([this.$download.get(0), this.$time.get(0)]) || $null;
			},
	
			initStyle: function(){
				var self = this;
				self.show(false);
				tvp.app.getAppMd5(self.op.promotionId,self.op.appName).done(function(json){
					if(json && json.md5){
						self.op.md5 = json.md5;
						json.url?self.op.downloadUrl = json.url:"";
					}
					if($.os.ios){
						self.op.downloadUrl = tvp.app.getDownloadUrl('', self.op.appName);
					}
					self.$btns.attr({
						'href': self.op.downloadUrl,
						'data-downloadurl': self.op.downloadUrl,
						'data-downloadmd5': self.op.md5,
						'data-promotionId': self.op.promotionId
					});
					self.checkBtn();
				});
			},
			...
			hasAppBanner: function(){
				...
				if($.createAppFollow ||
					self.t &&
					self.t.config && 
					self.t.config.plugins && 
					(self.t.config.plugins.AppBanner|| 
					 self.t.config.plugins.AppFollow) ){
					return true;
				}
				return false;
			},
			...
			isOpenCase3: function(){
				var self = this;
				if(this.op.disable==='all'){
					return false;
				}
				if(!$.isArray(self.op.disable)){
					return true;
				}
				var getPageType = function(){
					var pt = tvp.app.pageType;
					var type, host;
					try{
					...
					if(pt===1){
						...
						if(host==='mp.weixin.qq.com'){
							type = 1;
						}else if(host==='view.inews.qq.com'){
							// 微信新闻插件
							type = 2;
						}else{
							type = 3;
				...
				if($.inArray(gpt, self.op.disable)===-1){
					return true;
				}else{
					return false;
				}
			...
			hasPauseBanner: function(){
				...
				if(!self.isOpenCase3() ||
					...
					self.t.config && 
					self.t.config.plugins && 
					(self.t.config.plugins.AppBannerOnPause) ){
					return true;
				}
				return false;
			},
			...
			videoListener: function(){
				...
				this.t.$video.on('pause', function() {
					if (self.t.control && self.t.control.isTouching) {// 正在拖进度条
						return;
					}else if($.isFunction(self.t.isPlayingLoadingAd) && self.t.isPlayingLoadingAd()){ //正在播放广告
						return ;
					}
					var isend = this.currentTime > 0 && Math.ceil(this.currentTime) == Math.ceil(this.duration);
					self.hideControl();
					self.show(isend, 400);
	
					if(!self.hasPauseBanner()){
						if(self.limitCase===3&&
							self.info.has_click_download&&
							!self.hasAppBanner()){
							self.$time_btn.removeClass(hideCls);
							self.timer = null;
						}
					}
					if(self.timeRemainVal<6){
						self.$time.addClass(hideCls);
					}
				}).on('play', function() {
					self.hide();
				}).on('playing', function(){
					if(self.limitCase===1 || self.limitCase===2){
						if(!self.hasAppBanner() && self.info.has_click_download){
							self.$time_btn.addClass(hideCls);
						}else{
							!self.hasAppBanner() && self.$time_btn.removeClass(hideCls);
						}
						if(self.info && !self.info.has_click_download){
							self.$download.addClass(hideCls);
						}
					}
					setTimeout(function(){
						if($.isFunction(self.t.isPlayingLoadingAd) &&
							self.t.isPlayingLoadingAd()){
							self.$time.addClass('tvp_app_badge_fix');
						}else{
							self.$time.removeClass('tvp_app_badge_fix');
						}
					},20);
				}).on('ended', function(){
					self.show(true);
				...
				// 如果有暂停banner，就没有倒计时
				!self.hasPauseBanner()&&this.t.$video.on('timeupdate', function(){
					self.timeRemainVal = parseInt(this.duration-this.currentTime);
					if(self.hasAppBanner()){
						return;
					}
					if(self.info.has_click_download){
					...
					if(self.is_case3_dl_show( parseInt(this.currentTime,10) )){
						self.$time_btn.removeClass(hideCls);
			...
			show: function(isend, delay){
				...
				setTimeout(function(){
					...
					self.$download && self.$download.removeClass(hideCls);
				}, delay);
				if(isend){
					this.$replay && this.$replay.removeClass(hideCls);
					this.$play && this.$play.addClass(hideCls);
				}else{
					this.$replay && this.$replay.addClass(hideCls);
					this.$play && this.$play.removeClass(hideCls);
				}
				this.limitCase!==3 && this.$time_btn && this.$time_btn.addClass(hideCls);
				this.limitCase===2 && this.$case2_main.removeClass(hideCls);
				this.limitCase!==3 && this.t.trigger && this.t.trigger('tvp.durationlimit.show');
			},
			...
			hide: function(){
				...
				var self = this;
				if(this.limitCase===2){
					this.$case2_main.addClass(hideCls);
					return;
				}
				!this.info.has_click_download && this.$download.addClass(hideCls);
				this.$replay.addClass(hideCls);
				this.$play.addClass(hideCls);
				if(this.limitCase!==3){
					!self.hasPauseBanner()&& !self.hasAppBanner() && this.$time_btn.removeClass(hideCls);
				}else{
					!this.info.has_click_download && this.$time_btn.addClass(hideCls);
				}
				this.limitCase!==3 && this.t.trigger && this.t.trigger('tvp.durationlimit.hide');
			},
	
			// 判断case3场景下载按钮是否显示
			// time = 5s, 65s, 之后每隔两分钟显示
			is_case3_dl_show: function(time){
				var self = this;
				if(self.limitCase!==3){
					return false;
				}
			...
			timeRemain: function(){
				...
				var m = parseInt(this.timeRemainVal/60),
					s = this.timeRemainVal%60,
					s = s < 10 ? ('0'+s) : s,
					m = m < 10 ? ('0'+m) : m,
					txt = m+':'+s;
				this.$time_text.attr('data-text','还剩 ' + txt);
			},
	
			getCase: function(){
				var ua = navigator.userAgent.toLowerCase(), regx_list;
				if(window.g_limit_conf && window.g_limit_conf.case_regx){
					regx_list = window.g_limit_conf.case_regx;
				}else{
					regx_list = defaultConfig.case_regx;
				}
				var regx_pass = false;
				$(regx_list).each(function(idx, val){
					if(val.test(ua)){
						regx_pass = true;
						return true;
					}
			...
			fixBtn : function(isend) {
				var self = this;
				if(this.info.hasApp){
					self.$btns.attr({
						'data-status':'open',
						'data-url': self.info.openUrl,
						'href': self.info.openUrl
					});
					this.limitCase===1 && this.$download_text.attr({
						'data-text': self.op.texts.afterInstall_1
					});
				}else{
					self.$btns.attr({
						'data-url': self.info.openUrl,
			...
			checkBtn : function() {
				...
				tvp.app.check({
					vid : self.vid,
					downloadUrl : self.op.downloadUrl,
					md5 : self.op.md5
				}).done(function(rs) {
					if(!self.info){
						self.info = {};
					}
					if( !/from=\d+_/.test(rs.openUrl) ){
						rs.openUrl += "&from=" + self.op.promotionId + '_';
					}
					self.info.openUrl = rs.openUrl;
					if(rs){
						self.info.hasApp = rs.hasApp;
						self.fixBtn();
						if(this.limitCase!==2){
							self.bindDownloader(self.$boxs, self.$btns);
						}
						// 下载步奏上报
						if(self.info.hasApp){
							self.$btns.on(eventname, function(){
								self._reportStep(7, 3);
							});
							self._reportStep(6, 3);
						}else if($.os.ios || self.limitCase===2){
							self._reportStep(1, 1);
							self.$btns.on(eventname, function(){
								self._reportStep(2, 1);
							});
						}
					}
				});
			},
			bindDownloader : function(btnBox, btn) {
				var self = this;
				// var appbannerParams = (self.t.config.plugins && self.t.config.plugins.AppBanner) ? self.t.config.plugins.AppBanner : {};
				var appbannerParams = {
					downloader : self.op.downloader,
					downloadUrl : self.op.downloadUrl,
					md5 : self.op.md5,
					range: self.op.range
				};
				...
				var params = {
					...
					downloadBtn: btn,
					range: self.op.range,
					appName: self.op.appName,
					downloadMd5: self.op.md5
				};
				tvp.app.checkCanDownloader(self.info.hasApp, appbannerParams, params);
			},
			// 隐藏控制栏
			hideControl: function(){
				if(this.t.control
					&& this.t.control.$UILayer){
					this.t.control.$UILayer.addClass('tvp_controls_hide');
				}
			},
			_bindEvents : function() {
				var self = this;
				...
				var execPlay = function(is_replay){
					if(uiPlay && $.isFunction(uiPlay.trigger)){
						// videoTag.play();
						uiPlay.trigger(eventname);
					}else {
						is_replay && videoTag.load();
						videoTag.play();
					}
				}
				this.$play.on(eventname, function() {
					execPlay();
				});
				this.$replay.on(eventname, function() {
					execPlay(1);
				});
				this.downloadState();
				...
				if(tvp.app.bindTryOpenAppBanner){
					tvp.app.bindTryOpenAppBanner({
						$btn: self.$btns,
						rewriteText: tvp.$.noop
					});
				}		
			},
			downloadState: function(){
				var self = this;
				var $btn, $btn_text, $time_text;
				if(this.limitCase===2){
					return;
				}
				if(this.limitCase===1){
					$btn = self.$btns;
					$btn_text = self.$download_text;
					$time_text = self.$time_text;
				}else{
					$btn = self.$time_btn;
					$btn_text = self.$time_text;
				}
				var begin_hide_time = function(){
					// pad上计时常驻
					if(is_pad) return;
					self.timer && clearTimeout(self.timer);
					self.timer = null;
					self.timer = setTimeout(function(){
						self.$btns.addClass(hideCls);
					},10000);
				};
				var appdownload = function(status){
					status==='downloading'
						?(self.info.has_click_download = true):
						 (self.info.has_click_download = false);
					var _s;
					_s = status==='afterInstall'?'open':status;
					$btn && $btn.attr({
						'data-status': _s,
						'href': self.info.openUrl
					});
					$btn_text && $btn_text.attr('data-text', self.op.texts[status]);
					$time_text && $time_text.attr('data-text',self.op.texts['_'+status]);
					if(self.limitCase===3 &&
						(status==='install' || status==='pause')){
						begin_hide_time();
					}else{
						self.timer && clearTimeout(self.timer) && (self.timer=null);	
					}
				};
				$btn.on('tvp:appdownload:complete', function(){
					appdownload('install');
				})
				.on('tvp:appdownload:downloading', function(){
					appdownload('downloading');
				})
				.on('tvp:appdownload:pause', function(){
					appdownload('pause');
				})
				.on('tvp:appdownload:afterInstall', function(){
					appdownload('afterInstall');
				});
			},
	
			// 下载步奏上报
			_reportStep: function(step, _case) {
				var self = this;
				var t = self.t;
				var op = {
					cmd : 3537,
		...
		$.extend(tvp.html5skin, {
			html: (function() {
				return [
					...
					'			<i class="tvp_ico_ff_rw tvp_ico_ff"></i><span class="tvp_text tvp_overlay_bigben_text">0:00:00</span>',
	...
	(function(tvp, $) {
		...
		tvp.Html5UI.fn = tvp.Html5UI.prototype = {
					...
					if (!this.player.config.isHtml5ControlAlwaysShow) {
						this.$video.on(t.getClickName(), function(e) {
							...
							if (t.isHidden() && (t.videoTag.currentTime || t.overlayPlayClicked)) {
								t.show();
								t.beginHide(3e3);; //显示了控制栏以后倒计时8秒，8秒内啥都不做，直接关闭，除非点击了其他控制区域
							} else {
								t.hide();
							}
							e.preventDefault();
			...
			show: function() {
				...
				this.$control && this.$control.trigger &&this.$control.trigger("tvp:control:show");
			},
			isHidden: function() {
		...
		$.extend(tvp.Html5UI.fn, {
			buildoverlay: function(player, $video, $control, $UILayer) {
				...
				var _pfn = function(e) {
					...
					$video.trigger && $video.trigger("tvp:h5ui:playbtn:click");
					//如果没点过
					if (!isClicked) {
						isClicked = true;
		...
		$.extend(tvp.Html5UI.fn, {
			buildprogress: function(player, $video, $control, $UILayer) {
				...
				function handleMove(touchEvent) {
					...
					$control.trigger && $control.trigger("tvp:progress:touchstart", data);
					return data;
				}
				...
				function iosProcessHandler(){
					var totalWidth, x1, x2, x3, curWidth, toWidth,
						progressWidth;
					var hasTouch = $.os.hasTouch;
					var evt = {
						start: hasTouch? 'touchstart': 'mousedown',
						move : hasTouch? 'touchmove' : 'mousemove',
						end  : hasTouch? 'touchend'  : 'mouseup'
					};
					$control.find('.tvp_time_rail').on(evt.start, function(e){
						e = !! e.originalEvent ? e.originalEvent : e;
						if(hasTouch && !(e && e.touches && e.touches.length)){
							return;
						}
						var evt;
						if(hasTouch){
						...
						$control.trigger && $control.trigger("tvp:progress:touchstart", {
							"pos": toWidth,
							"precent": per,
							"time": toTime
						});
						...
						player.seek(toTime);
						$control.trigger && $control.trigger("tvp:progress:touchend");
						t.isTouching = false;
						t.player.isTouching = false;
						e.preventDefault();
						e.stopPropagation();
					});
				}
				...
				$video.bind("progress", function(e) {
					...
					if (videoTag.buffered && videoTag.buffered.length > 0 && videoTag.buffered.end && player.getDuration()) {
						...
						curLeft = videoTag.buffered.end(0) / player.getDuration();
						curLeft = parseInt(curLeft*100) + '%';
	...
	(function(tvp, $) {
		...
		$.extend(tvp.Html5UI.fn, {
			...
			userActionReport: function(opt){
				...
				var g_btrace_BOSS = new Image();
				var url = "http://btrace.qq.com/kvcollect?BossId=2797&Pwd=219856622&_dc=" + Math.random();
				url += "&" + $.param(params);
				g_btrace_BOSS.src = url;
			},
			...
			listenOrientationChange: function(){
			    ...
			    player.$videomod.css({
					...
					// 如果有结束推荐，重新计算滚动值
					t.$mod && t.$mod.trigger && t.$mod.trigger('tvp:recommend:orientationchange');
					if(t.$video && t.$video.trigger){
						setTimeout(function(){
							t.$video.trigger('timeupdate');
							t.$video.trigger('progress');
						},100);
					}
				};
				try{
			...
			buildposterlayer: function(player, $video, $control, $UILayer) {
				var $poster = $UILayer.find(tvp.html5skin.elements.posterlayer.main),
					...
					showPoster = function() {
						if(player 
							&& player.control 
							&& player.control.checkIsFullScreen
							&& !player.control.checkIsFullScreen()){
							$poster.show();
						}
	...
	(function(tvp, $) {
		$.extend(tvp.Html5UI.fn, {
			...
			buildshadow: function(player, $video, $control, $UILayer) {
				var $shadow = $('<div class="tvp_shadow"></div>').appendTo($UILayer.find('.tvp_video'));
				var t = this;
				//如果不是设置为永远显示控制栏，就要做自动化隐藏逻辑
		...
		function init(player, $video, $control, $UILayer){
			var adPlayer = new tvp.Html5LoaingAd(),
				...
				adPlayer.onEnd = function(){
					...
					$video.trigger && $video.trigger("tvp:loadingad:ended");
	...
	(function(tvp, $) {
		...
		function loadModule(playerClass, getFileReport) {
			...
			var url = jsurl + "module/" + jsfile + ".js?max_age=86400&v="+tvp.ts;
		...
		var create = function(config, player) {
			...
			function useDefaultVodPlayer() {
				var platform = navigator.platform.toLowerCase(),
					...
					// flashVer = tvp.version.getFlashMain(),
					useH5ByUA = /ipad|ipod|iphone|lepad_hls|IEMobile|WPDesktop/ig.test(ua),
					useH5ByPltf = /ipad|ipod|iphone|linux/ig.test(platform);
				if (tvp.common.isEnforceMP4()) { // 有些浏览器强制走MP4
					playerClass = "MP4Link";
					return;
				}
				// ios等设备，linux平台设备，优先H5
				// 不支持H5，直接mp4link
				if (useH5ByUA || 
					useH5ByPltf ||
					tvp.$.os.android ) {
					if(tvp.common.isSupportMP4() || tvp.common.isInUseH5()){
						useWhichVodHtml5();
					}else{
						playerClass = "MP4Link";
					}
			...
			$.when(config.type == tvp.PLAYER_DEFINE.VOD ? checkVodPlayer() : checkLivePlayer(config.video)).then(function() {
				        ...
				        if (config.type == tvp.PLAYER_DEFINE.VOD && isUseVodHtml5) {
					        player && player.trigger && player.trigger(tvp.ACTION.onVodH5Init);
				        }
				        if (typeof tvp[playerClass] !== "function") {
	...
	(function(tvp, $) {
		...
		$.extend(AppBanner.prototype, {
			...
			fixTpl: function() {
				...
				if(op.btnText === defaultConfig.btnText &&
					op.appName === 'weishi'){
					op.btnText = op.btnText2;
				}
			...
			getCurPromotionId: function(config){
				...
				// 取参数里url的渠道号
				if(!id && config.downloadUrl){
					id = tvp.$.getUrlParam('confid',config.downloadUrl);
				}
				// 取参数里的渠道号
				if(!id && config.promotionId){
					id = config.promotionId;
				}
			...
			fixDownloadStatus : function(type) {
				var name = "下载",
					op = this.op,
					textName = defaultConfig.appTextName[op.appName],
					textName2;
				if(!textName){
					textName = defaultConfig.appTextName[tvp.app.config.defaultName];
				}
				if(op.appName==='weishi'){
					textName2 = '发现更多精彩';
				}else{
					textName2 = '观看更多';
				}
	...
	(function(tvp, $) {
		$.extend(tvp.Player.fn, {
			...
			buildAppBanner: function(config) {
				...
				$videomod.trigger && $videomod.trigger('tvp:resize');
	...
	(function(tvp, $) {
		...
		var hasApp = false, hasReportShow=false, swipeHistory={0:null};
	
		/**
		 * 获取当前下载文件的渠道号
		 */
		function getCurPromotionId(config){
			var id;
			var t = config.t;
			if(config.relatedBtn){
				if(hasApp){
					id = tvp.$.getUrlParam('from',config.relatedBtn.attr('href'));
					id = id.replace(/\D/g,'');
				}else{
					id = tvp.$.getUrlParam('confid',config.relatedBtn.attr('data-downloadurl'));
				}
			}
			if(!id && config.promotionId){
				id = config.promotionId;
		...
		function _reportStep(step, config) {
			var op = {
				cmd : 3537,
				int5 : step,
				str4 : hasApp ? 3 : 1,
				str8 : $.getUrlParam('mmuin'),
				val : config.initStep || 1
			};
			var t = config.t;
			op.int6 = getCurPromotionId(config);
			tvp.app.report(op, t);
		}
		...
		$.extend(AppRecommend.prototype, {
			...
			fixUI : function() {
				...
				var t = op.t;
				var $mod = op.$mod;
				var $mod_video = $mod.find('.tvp_video');
				if(!t.$UILayer) {
					$mod.addClass('tvp_container');
					var shadow = $mod_video.find('.tvp_shadow');
					if(!shadow.length) {
						shadow = $('<div class="tvp_shadow"></div>').appendTo($mod_video);
						shadow.hide();
					}
					this.$shadow = shadow;
				}
			...
			move : function(direction) {
				...
				// 结束推荐滑动曝光上报
				if(!swipeHistory.hasOwnProperty(toIndex)){
					_reportStep((hasApp?6:1), op);
					swipeHistory[toIndex] = null;
				}
			},
				...
				.on('touchend', function(e){
					...
					if(m<25){
						setTimeout(function(){
							if(!clicked){
								var $el = $(elem);
								$el && $el.trigger && $el.trigger('click');
								clicked = null;
							}
						},380);
						evt = null;
